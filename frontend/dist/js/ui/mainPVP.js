...
import { getGameState } from "../core/state.js";
import { renderTokens } from "../core/boardRenderer.js";
import { redPathHead, bluePathHead, getNodeByStep, } from "../core/boardPath.js";
const state = getGameState();
const sessionId = state.sessionId;
...
async function syncGameState() {
    try {
        ...
        if (!res.success)
            return;
        players = res.players?.map((p) => Number(p)) ?? players;
        turnIndex = Number(res.turn ?? 0);
        const currentPlayer = players[turnIndex];
        myId = Number(res.me);
        const isMyTurn = myId === currentPlayer;
        ...
        if (res.game_state && res.game_state.tokens) {
            const allTokens = res.game_state.tokens;
            Object.entries(allTokens).forEach(([_, tokenList]) => {
...
// ========================================================
function updateTokenPosition(tokenId, steps) {
    const isRed = tokenId.startsWith("P1_");
    const pathHead = isRed ? redPathHead : bluePathHead;
    const node = getNodeByStep(pathHead, steps);
    if (!node)
...
// Reset Token to Yard
// ========================================================
function resetTokenToYard(tokenId) {
    const tokenEl = document.getElementById(tokenId);
    if (!tokenEl)
        return;
    let circleId = "";
    if (tokenId.startsWith("P1_"))
        circleId = "rc" + tokenId.split("_T")[1];
    else if (tokenId.startsWith("P2_"))
        circleId = "bc" + tokenId.split("_T")[1];
    const targetCircle = document.getElementById(circleId);
...
// ========================================================
function moveTokenToHome(tokenId) {
    const tokenEl = document.getElementById(tokenId);
    if (!tokenEl || tokenEl.dataset.inHome === "true")
        return;
...
// ========================================================
export async function handleTokenClick() {
    const tokenId = this.id.trim();
    if (!tokenId)
        return;
    try {
        const session = await getSession(sessionId);
        const players = session.players.map((p) => Number(p));
        const turnIndex = Number(session.turn ?? 0);
        const myId = Number(session.me);
        const currentPlayer = players[turnIndex];
...
// ========================================================
function frontendMoveToken(tokenId, steps) {
    const isRed = tokenId.startsWith("P1_");
    const pathHead = isRed ? redPathHead : bluePathHead;
    const tokenEl = document.getElementById(tokenId);
    if (!tokenEl)

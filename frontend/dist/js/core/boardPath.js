...
export function buildPath(coords) {
    if (!coords.length)
        throw new Error("No coordinates provided for path.");
 ...
 */
export function convertToArray(head) {
    const arr = [];
    let current = head;
    while (current) {
 ...
 */
export function getNodeByStep(head, step) {
    let current = head;
...
export function printPath(head) {
    let current = head;
    const pathCoords = [];
    while (current) {
 ...
 */
export function getPositionByStep(player, step) {
    const pathArray = player === "red" ? redPathArray : bluePathArray;
    return pathArray[step] || null;
}
export function getNextPositionByStep(player, currentStep, stepsToMove) {
    const pathArray = player === "red" ? redPathArray : bluePathArray;

 ...
 */
export function highlightMovableTokens(coords: { x: number; y: number }[]): void {
  coords.forEach(({ x, y }) => {
    const selector = `.cell[data-x='${x}'][data-y='${y}'] .inner_circle`;
    const circle = document.querySelector(selector) as HTMLElement | null;
    if (circle) {
 ...
 */
export function moveTokenVisual(player: "red" | "blue", x: number, y: number): void {
